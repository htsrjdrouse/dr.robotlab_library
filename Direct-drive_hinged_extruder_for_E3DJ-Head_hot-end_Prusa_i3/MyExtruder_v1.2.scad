...
difference() {
	...
	union() {
		...
		translate([x_filament,35-50,z_filament]) rotate([90,0,0]) cylinder(r=4.5/2, h=2+13, $fn=64);
                for(i=[0:10]){
		translate([x_filament,35-54,z_filament+(i*0.5)]) rotate([90,0,0]) cylinder(r=4.5/2, h=2+13, $fn=64);
  	 	}
		...
		translate([-17.5,-28+3.6,10]) rotate([0,90,0]) #cylinder(r=3.8/2, h=8, $fn=64);	
...
//mk8();
base();
//translate([0,0,1]) rotate([0, 180, 0]) hot_end_mount();
//translate([0,0,2])hot_end_mount();
