...
module rpi4_case()
{
  difference(){ // subtracts the rpi4 model from a cube to generate the case
    ...
    cube([pil+(2*case_thickness), pid+(2*case_thickness),  pin_space+inhibitionzone_height+board_thickness+(2*case_thickness)]); // the case itself
    translate([-20,-5,13])cube([150,65,3]);
    union(){
      rpi4();
